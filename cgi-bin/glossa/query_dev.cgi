...
#	if($CORPUS eq 'demo'){ $ex_url .= "&db=glossa&table=DEMOsegments";  }

	if ($speech_corpus) {
#javascript:top.frames.inspect.location='http
#	    print "<script>alert('[[" . $identifier . "]]')</script>";
...
#	    $source_line.=sprintf("<font size=\"-2\">\n<a href=\"#\" onClick=\"window.open('http://omilia.uio.no/cgi-bin/glossa/expand.pl$ex_url&video=0',");
#	    $source_line.=sprintf("'mywindow','height=400,width=1000,status,scrollbars,resizable,screenX=0,screenY=5');\">\n<img style='border-style:none' src='http://omilia.uio.no/glossa/html/img/mov.gif'>\n</a> \n&nbsp;</font>");
#	    $source_line.=sprintf("<font size=\"-2\">\n<a href=\"#\" onClick=\"window.open('http://omilia.uio.no/cgi-bin/glossa/expand.pl$ex_url&video=audio',");
...
foreach my $i (1..$d_files) {
    my $id = "page_" . $i;
    print TOP " <a id=\"$id\" href=\"$conf{'cgiRoot'}/show_page_dev.cgi?n=$i&query_id=$conf{'query_id'}&corpus=$CORPUS&atttype=$atttype\">$i</a> ";
}
print "</div>";
print "</body></html>\n";
